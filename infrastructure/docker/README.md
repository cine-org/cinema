...
Redis is optional for the current application wiring. `pnpm docker:infra` starts only the
database stack; use `pnpm docker:redis` when cache or queue work needs local Redis.
